        ...
        <div class="card shadow border-0 rounded-4 mb-4">
            ...
            <div class="card shadow-lg border-0 rounded-4">
                <div
                    ...
                    <h5 class="mb-0">เรื่อง : {{ $list->corruptions_topic }}</h5>
                    @if ($list->corruptions_status == 'R')
                ...
                <div class="card-body p-4">
                        ...
                        <table class="table table-bordered align-middle">
                            <tbody>
                                <tr>
                                    ...
                                    <td>{{ $list->corruptions_name ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">อายุ</th>
                                    <td>{{ $list->corruptions_age ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">เพศ</th>
                                    <td>{{ $list->corruptions_gender ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">ที่อยู่</th>
                                    <td>{{ $list->corruptions_address ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">เบอร์ติดต่อ</th>
                                    <td>{{ $list->corruptions_tel ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">E-mail</th>
                                    <td>{{ $list->corruptions_email ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">รายละเอียด</th>
                                    <td>{!! nl2br(strip_tags($list->corruptions_detail)) ?? '-' !!}</td>
                                </tr>
                                <tr>
                                    <th class="bg-light">ภาพประกอบ</th>
                                    <td>
                                        @if (!empty($list->corruptions_file))
                                            <img src="{{ asset('storage/' . $list->corruptions_file) }}"
                                ...
                                <tr>
                                    ...
                                    <td>{{ $list->corruption_date_show_buddhist }}</td>
                                </tr>
                                @if (!empty($list->corruptions_note))
                                    <tr>
                                        <th class="bg-light">หมายเหตุ</th>
                                        <td>
                                            {{ $list->corruptions_note }}
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
                    ...
                    @if ($list->corruptions_status == 'N')
                        <form action="{{ route('corruptiondetail.update', ['menu' => $menuId, 'id' => $id]) }}"
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="note" id="note"
                                    placeholder="กรอกหมายเหตุ" value="{{ $list->corruptions_note ?? '' }}" required>
                            </div>
